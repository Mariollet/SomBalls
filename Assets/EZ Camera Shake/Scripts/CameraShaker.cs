...
namespace EZCameraShake
{
    ...
    public class CameraShaker : MonoBehaviour
    {
        ...
        /// </summary>
        public Vector3 RestRotationOffset = new Vector3(10, 0, 0);
        ...
        void FixedUpdate()
        {
            if (gameManager.gameOver == false)
            {
                ...
                RestPositionOffset = smoothedPosition;

                if (RestPositionOffset.x > bound)
                {
                    RestPositionOffset = new Vector3(bound, RestPositionOffset.y, RestPositionOffset.z);
                }
                if (RestPositionOffset.x < -bound)
                {
                    RestPositionOffset = new Vector3(-bound, RestPositionOffset.y, RestPositionOffset.z);
                }
                if (RestPositionOffset.y > boundY)
                {
                    RestPositionOffset = new Vector3(RestPositionOffset.x, boundY, RestPositionOffset.z);
                }
            }
